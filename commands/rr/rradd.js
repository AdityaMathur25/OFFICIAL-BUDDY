...
module.exports = {
    ...
    async run(client, message, args, flags) {
          const invalidEmbed = new Discord.MessageEmbed()
            ...
            const check = message.guild.roles.cache.find(r => r.name === `${role.name}`);
            ...
            client.channels.cache.get(`${channel.id}`).messages.fetch(`${args[1]}`).then(a => {
                ...
                a.react(emojicheck.id)
                    ...
                    rr.set(`rrremove_${message.guild.id}_${args[1]}_${args[3]}`, emojicheck.id);

                    rr.set(`rerremove_${message.guild.id}_${args[1]}`, args[1]);

                    rr.set(`emoteid_${message.guild.id}_${emojicheck.id}`, emojicheck.id);
